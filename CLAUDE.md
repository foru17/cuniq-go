...
- **号池保留窗口按池分开**（`ordinaryWindowMs` / `specialWindowMs`）：
  - cuniq 两个池都是 45 分钟（约 3 轮）——10 批只能覆盖大部分而非全部，实测连续几轮靓号池在 723~906 之间来回跳；窗口为 0 时这些仍在售的号会每 15 分钟消失又出现。
  - cmhk 普通池（W）3h，靓号池（C/D）0——C/D 每次调用返回**全量**列表，缺席即已售；W 是大池里的批次，缺席不代表售出。
- **逐号可用性校验**（cmhk 用 `verifyCmhkNumbers`，把 8 位号码填进 `msisdnCondition`；cuniq 用 `verifyCuniqNumbers`，用 `queryNum=<8位>`，两个池的 plan id 不同必须查对应 URL）：命中=在售、空=已售。每轮对"本轮没见到且还在窗口内"的号按最旧优先校验，预算 `verifyBudget=40` 在有窗口的池之间平分，`gone` 删除、`available` 续期、任何异常算 `unknown` 一律保留。单轮上游调用硬上限由 `maxUpstreamCalls` 约束（cmhk 50、cuniq 60）。
